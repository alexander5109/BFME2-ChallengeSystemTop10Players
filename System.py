...
class ChallengeSystem:
	...
	###--------------------------Public.Methods-----------------------###
	def consult_01_challenge_log(reverse=False):
		sorted_dict = {key: self.CHALLENGES[key] for key in sorted(self.CHALLENGES, reverse=reverse)}
		for cha in sorted_dict.values():
			print(cha)
			
	def consult_03_player_vs_player(p1_key, p2_key, print_em):
		return self.PLAYERS[p1_key].get_1v1_vs(self.PLAYERS[p2_key], printEm=print_em)
		
	def consult_04_who_is_black(pname):
		ic(self.PLAYERS[pname].is_black())
	
	def consult_05_2v2_score(pname):
		ic(sistema.PLAYERS[pname].wins2v2_total)
		ic(sistema.PLAYERS[pname].loses2v2_total)

	
	###--------------------------Private.Methods-----------------------###
	def __write_csv(data, reverse=False):
		if reverse:
	...
	def __write_chalog(self):
		sorted_dict = {key: self.CHALLENGES[key] for key in sorted(self.CHALLENGES, reverse=True)}
		...
		for num, cha in enumerate(sorted_dict.values(), start=1):
			if num == 1:
				self.__rename_folder("torename", cha.replays_folder_name, compress=False)
				print(cha)
	...
	def __compress_folder(self, folder_path):
	...
	def __rename_folder(self, torename, ideal, compress):
